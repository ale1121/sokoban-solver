...
class Map:
    ...
    def can_push_box(self, box, reachable_positions):
        box_x, box_y = box
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            behind = (box_x + dx, box_y + dy)
            ahead = (box_x - dx, box_y - dy)

            if behind in reachable_positions and self.is_valid_position(ahead):
                return True
            
        return False
    
    def is_near_box(self, pos):
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = pos[0] + dx, pos[1] + dy
            if (nx, ny) in self.positions_of_boxes:
                return True
        return False
    ...
    def filter_possible_moves(self, box_moves_only=False):
        ...
        possible_moves = []
        for move in range(start, BOX_DOWN + 1):
    ...
    def get_neighbours(self, box_moves_only=False):
        ''' Returns the neighbours of the current state'''
        neighbours = []
        for move in self.filter_possible_moves(box_moves_only):
            new_map = self.copy()
            new_map.apply_move(move)
            neighbours.append(new_map)
        return neighbours
    
    def get_reachable_positions(self, ignore_boxes=False):
        visited = set()
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if (nx, ny) in visited:
                    ...
                    if self.is_valid_position((nx, ny)):
                        queue.append((nx, ny))
                elif self.is_valid_position((nx, ny)) and (nx, ny) not in self.positions_of_boxes:
                    queue.append((nx, ny))
